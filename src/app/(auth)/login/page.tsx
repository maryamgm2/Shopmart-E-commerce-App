...
export default function Login() {
  ...
  async function handleLogin(values: loginSchemaType) {
    ...
    if (response?.ok) {
      toast.success("Logged in Successfully", { position: "top-center", duration: 5000 })
      router.push("/products")
    } else {
      toast.error(response?.error || "Invalid email or password", { position: "top-center", duration: 5000 })
    }
  }
    ...
    <main className='min-h-screen flex items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-8'> 
      <div className='w-full max-w-md space-y-8 bg-white p-6 sm:p-10 rounded-[32px] shadow-sm border border-gray-100'>
        <div className="text-center sm:text-left">
          ...
          <p className='mt-2 text-sm sm:text-base font-medium text-gray-500'>Login Now!</p>
        </div>
        ...
        <div className="mt-8">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleLogin)} className='space-y-6'>
              ...
              <Button type="submit" disabled={form.formState.isSubmitting} className='w-full h-12 cursor-pointer bg-black text-white transition-all duration-300 rounded-xl py-6 border border-black hover:bg-white hover:text-black hover:shadow-md active:scale-[0.98]'>
